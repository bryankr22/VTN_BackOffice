...
export default function CreateTechnicalSheets({ data, models }) {
  ...
  const [technicalSheets, setTechnicalSheets] = useState(data.technical_sheets);
  ...
  const updateTechnicalSheets = async () => {
    setLoading(true);
    ...
    Object.entries(technicalSheets).map((item) => {
      if (!item[1] && item[0] !== 'views' && item[0] !== 'active') {
        error = true;
        return;
      }
    });
    ...
    const res = await axios.post(`${API_URL}/updated-technical-sheets`, formData, config);
    setSnackBar({
      open: true,
        ...
        <Grid container spacing={2} style={{ marginTop: 15 }}>
          ...
          <Grid item xs={12} md={12}>
            <Box
              ...
              autoComplete="off"
            >
              <LoadingButton loading={loading} fullWidth size="large" onClick={updateTechnicalSheets} variant="contained">Guardar</LoadingButton>
            </Box>
          </Grid>
        </Grid>
      </>
...
export async function getServerSideProps(context) {
  ...
  const res = await axios.get(`${API_URL}/form-updated-technical-sheets/${context.query.id}`, config);
  const data = await res.data;
  const models = data.modelos.filter((obj) => obj.marca_id === data.technical_sheets.mark_id);
