...
export default function Promotion({ data }) {
  ...
  const deletePromotion = async (id) => {
    ...
    const res = await axios.post(`${API_URL}/delete-promotion`, { id }, config);
    setSnackBar({
      open: true,
      type: res.data.status ? 'success' : 'error',
      message: res.data.message
    });
    setRows(res.data.promotion);
    setOpenAlert(false);
    setLoading(false);
  }
      ...
      renderCell: (cellValues) => {
        return (
          <Checkbox
            ...
            onChange={async () => {
              setLoading(true);
              const activePromotion = await inactive(cellValues.row.id);
              cellValues.row.active = activePromotion;
              document.getElementById("promotion-title").click()
              setLoading(false);
            }}
        ...
        return (
          <>
            <Button
              ...
              onClick={() => {
                location.href = `/promotion/${cellValues.row.id}`;
              }}
            >
              Editar
            </Button>
